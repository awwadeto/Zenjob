...
class OfferViewModelTests: XCTestCase {
  ...
  override func setUp() {
    ...
    sut = OfferViewModel(dispatcher: dispatcher, user: mockUser, offer: mockOffer)
  }
  ...
  func testFetchOfferOnSuccess() {
    ...
    XCTAssertEqual(sut.offer.id, "g3pfhmFPmimg11")
  }

  func testFetchOfferOnError() {
    sut.offerTask.id = "i_TBkrB9g7uQ"
    let expectation = self.expectation(description: "fetchOffer")
    sut.fetchOffer {
      expectation.fulfill()
    }
